...
def set_company_research(conn: sqlite3.Connection, job_id: str, blurb: str) -> None:
    """Cache a company-research blurb. See models.Job.company_research and
    company_research.py."""
    conn.execute(
        "UPDATE jobs SET company_research = ? WHERE id = ?",
        (blurb, job_id),
    )
    conn.commit()
