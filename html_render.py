...
def _mark_svg(size: int = 34) -> str:
    """The pen-nib mark (north-up) used in the masthead — an unfilled kite
    with a center ink-slit and a breather-hole circle, the two details that
    read as "nib" rather than just "diamond outline"."""
    h = round(size * 1.16)
    return (
        f'<svg width="{size}" height="{h}" viewBox="0 0 100 100" aria-hidden="true">'
        f'<polygon points="50,8 68,44 50,90 32,44" fill="none" stroke="{BLUEPRINT}" stroke-width="4"></polygon>'
        f'<line x1="50" y1="50" x2="50" y2="84" stroke="{BLUEPRINT}" stroke-width="2.4"></line>'
        f'<circle cx="50" cy="26" r="4.2" fill="none" stroke="{BLUEPRINT}" stroke-width="2.4"></circle>'
        f'</svg>'
    )
...
def page(title: str, intro: str, body: str, *, head_extra: str = "",
    ...
    backgrounds/scripts) gets the grid-paper ground, a mark+wordmark masthead
    with a faded contour backdrop, corner registration ticks, and a drafting
    title-block footer. `chrome=False` (the emailed digest) stays on a flat
    background with a plain heading/footer and no canvas/script — Gmail
    strips embedded `background-image` gradients and won't run script at
    all, so the digest never relies on either."""
    grid_bg = (
    ...
    corners = "".join(
        f'<span style="position:absolute;{v}:12px;{h}:12px;width:8px;height:8px;'
        f'background:{BLUEPRINT};z-index:1;"></span>'
    ...
    heading = (
        ...
        f'<div style="font-family:{FONT_MONO};font-size:10.5px;font-weight:600;'
        f'letter-spacing:0.14em;text-transform:uppercase;color:{MUTED};margin-top:2px;">'
        f'{_esc(view).upper()}</div></div></div>'
        f'<div style="font-family:{FONT_MONO};font-size:11px;color:{MUTED};">'
        f'{date.today().isoformat()}</div></div></div>'
    ) if chrome else (
    ...
    return (
        ...
        f'color:{MUTED};font-size:14px;margin:12px 0 16px;">{intro}</div>'
        f'{body}'
        f'{footer}'
        f'</div></body></html>'
    )
